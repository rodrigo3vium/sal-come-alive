...
const faqs = [
  ...
  },
  {
    ...
    answer: 'Para SALtar conosco, basta clicar em um dos botões "QUERO SALTAR" ou "QUERO VOAR" dentro da página do serviço selecionado ou entrar em contato pelo WhatsApp https://wa.me/message/4GEMZDTURP6EE1 ou e-mail atendimento@studiosal.com.br. Com esse primeiro contato, iremos te conhecer melhor para entender se de fato faz sentido trabalharmos juntas. Somos uma agência boutique, comprometida com um atendimento personalizado, próximo e artesanal. Devido à alta demanda, precisamos selecionar com carinho os projetos que melhor se alinham com a transformação que podemos promover. Acreditamos que você nos escolhe, mas nós também escolhemos você! Por isso, ressaltamos que nem todos os projetos que recebemos poderão ser aprovados para fazerem parte da Sal.',
  },
  {
...
const FAQSection = () => {
  return (
    <section id="faq" className="bg-beige py-11 md:py-20">
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {faqs.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`} className="border border-foreground/20 rounded-none px-6 bg-transparent">
              <AccordionTrigger className="font-body text-left text-base text-foreground/70 hover:no-underline py-4 font-normal tracking-wide">
                {faq.question}
              </AccordionTrigger>
